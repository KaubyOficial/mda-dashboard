...
const TAB_LABELS: Record<string, string> = {
  ...
  leadsComercial: 'LEADS COMERCIAL',
};
...
/**
 * Abas OPCIONAIS: ausência NÃO derruba o sync (a LEADS COMERCIAL só existe depois de rodar
 * `npm run comercial:init`). Presente → lida no mesmo batchGet; ausente → aviso e lista vazia.
 */
const OPTIONAL_TABS = ['leadsComercial'] as const;
...
export class SheetSource implements DataSource {
  ...
  /** sheet-csv: uma requisição por aba, em paralelo. Opcionais só entram se tiverem gid. */
  private async fetchAllCsv(): Promise<Map<string, string[][] | null>> {
    const out = new Map<string, string[][] | null>();
    const keys = [...TAB_ORDER, ...OPTIONAL_TABS.filter((k) => this.opts.gids[k])];
    const results = await Promise.all(
      keys.map((key) => this.fetchTabCsv(this.opts.gids[key] ?? '', this.tabName(key))),
    );
    keys.forEach((key, i) => out.set(key, results[i] ?? null));
    return out;
  }
  ...
  private async fetchAllApi(warnings: string[]): Promise<Map<string, string[][] | null>> {
    ...
    // Abas opcionais: presentes → entram no mesmo batchGet; ausentes → aviso, nunca erro.
    const optionalKeys: string[] = [];
    for (const key of OPTIONAL_TABS) {
      const wanted = this.tabName(key);
      const actual = byNormalized.get(normalizeTabName(wanted));
      if (actual) {
        resolved.set(key, actual);
        optionalKeys.push(key);
      } else if (key === 'leadsComercial') {
        warnings.push(
          `COMERCIAL: a aba "${wanted}" ainda não existe na planilha — a seção Comercial fica sem a lista de leads. Criar com: npm run comercial:init --workspace server.`,
        );
      }
    }

    const allKeys = [...TAB_ORDER, ...optionalKeys];
    const titles = allKeys.map((k) => resolved.get(k) as string);
    ...
    for (const key of allKeys) {
      ...
      // aba opcional recém-criada só com o header não é anomalia — não avisa
      if (rows.length === 0 && !optionalKeys.includes(key))
        warnings.push(`Aba "${title}" veio vazia da API.`);
      out.set(key, rows);
    }
    return out;
  }
  ...
  async fetchAll(): Promise<DataSnapshot> {
    ...
    const lcRows = tabs.get('leadsComercial') ?? null;
    const leadsComercial = lcRows ? parseLeadComercialRows(lcRows, warnings) : [];
